...
class Character:
    def __init__(self,name,soldier_type="Soldier",health=100,strength=20):
        ...
        self.move3,self.move4,self.move5,self.move6 =0,0,0,0
                ...
                if 1 <= n <= 6:
                    if self.potion_quantity[n]>0:
                        ...
                        if n == 1:
                            ...
                            self.potion5 = True
                            self.potion_quantity[n] -= 1
                            print(f"{self.name} Increase critical hit probability (+50%)\n")
                        else:
                            self.potion6 = True
                            self.potion_quantity[n] -= 1
    ...
    def round(self, ch1: Character, ch2: Character,num):
        while True:
            ...
            if ch1.health<=0 and ch2.health<=0:
                time.sleep(1)
                print(f"Both {ch1.name} and {ch2.name} died😭")
                return True
            elif ch1.health<=0:
                time.sleep(1)
                print(f"{ch2.name} WINS!!!😍")
                return True
            elif ch2.health<=0:
                time.sleep(1)
                print(f"{ch1.name} WINS!!!😍")
                return True
            return False
    def run(self):
        while True:
